...
#ifndef pty_sharp_h_included
#define pty_sharp_h_included
...
int _pty_sharp_open(pid_t *child, char **env_add,
...
int _pty_sharp_get_size(int master, int *columns, int *rows);
int _pty_sharp_set_size(int master, int columns, int rows);

/* Try to let the kernel know that the terminal is or is not UTF-8. */
void _pty_sharp_set_utf8(int pty, gboolean utf8);

/* Close a pty. */
void _pty_sharp_close(int pty);
